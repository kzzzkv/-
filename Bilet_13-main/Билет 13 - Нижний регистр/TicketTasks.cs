...
{
    public const string FirstTaskTitle = "Нижний регистр";
    public const string SecondTaskTitle = "Поиск минимального значения";

    public static string ToLowerCase(string text) => text.ToLowerInvariant();

    public static int FindMin(IEnumerable<int> numbers) => numbers.Min();

    public static string RunFirstDemo(string valueA, string valueB)
    {
        return $"Lowercase: {ToLowerCase(valueA)}";
    }
