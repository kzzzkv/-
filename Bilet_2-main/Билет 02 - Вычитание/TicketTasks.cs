...
{
    public const string FirstTaskTitle = "Вычитание";
    public const string SecondTaskTitle = "Проекция (возведение в квадрат)";

    public static double Subtract(double a, double b) => a - b;

    public static int[] SquareNumbers(IEnumerable<int> numbers) =>
        numbers.Select(number => number * number).ToArray();
    ...
    public static string RunSecondDemo(string valuesText, string parameterText)
    {
        ...
        return $"Squares: {Format(SquareNumbers(numbers))}";
    }
