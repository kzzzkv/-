...
{
    public const string FirstTaskTitle = "Проверка начала строки (начинается с…)";
    public const string SecondTaskTitle = "Проверка всех элементов";

    public static bool StartsWithText(string text, string prefix) =>
        text.StartsWith(prefix, StringComparison.OrdinalIgnoreCase);

    public static bool AreAllPositive(IEnumerable<int> numbers) =>
        numbers.All(number => number > 0);

    public static string RunFirstDemo(string valueA, string valueB)
    {
        return $"Starts with '{valueB}': {StartsWithText(valueA, valueB)}";
    }

    public static string RunSecondDemo(string valuesText, string parameterText)
    {
        int[] numbers = ParseInts(valuesText);
        return $"All positive: {AreAllPositive(numbers)}";
    }
